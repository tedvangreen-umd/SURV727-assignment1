...
There is an extra column in the angell_txt version. There don't appear to be differences between the classes of the columns in each one. There are no column names for the txt file.
...
I added column names to the txt file, as well as dropped the column of NAs from that file as well.
...
By region, the south has a higher mean and median than other regions, while the west has a slightly lower value on both. The south has a much higher standard deviation.
...
Boston$crime_bin <- ifelse(Boston$crim>1, "Crime rate >1","Crime rate <1")
...
Boston %>% 
  group_by(crime_bin) %>% 
  summarize(min=min(ptratio),
            max = max(ptratio),
            mean = mean(ptratio),
            median = median(ptratio),
            IQR = IQR(ptratio))
...
my_CI4 <- function(col_name, ci_level){
...
my_CI5 <- function(dataset){
  output <- NULL
  #subsetting so this runs on each column
  ...
  output <- rbind(output, c(i, lower, upper))
  }
  else {
    output <- rbind(output, c(i, NA, NA))}
  }
  colnames(output) <- c("var", "lower bound", "upper bound")
  return(output)
}
...
Here is that function applied to the Angel dataset.
